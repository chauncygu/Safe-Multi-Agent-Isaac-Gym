...
class Runner:
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                # Sample actions
                actions, share_actions = self.collect(step)
                ...
                data = obs, share_obs, rewards, next_obs, next_share_obs, actions, share_actions, dones, infos
    ...
    def collect(self, step):
        ...
        share_actions = torch.cat(action_collector,dim =-1)

        return action_collector, share_actions

    def insert(self, data):
        obs, share_obs, rewards, next_obs, next_share_obs, actions, share_actions, dones, infos = data


        for agent_id in range(self.num_agents):
            self.buffer[agent_id].insert(obs[:, agent_id], share_obs[:, agent_id], actions[:, agent_id],
                                         share_actions[:, agent_id], rewards[agent_id],
                                         next_obs[agent_id],
