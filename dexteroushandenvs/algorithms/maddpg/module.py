...
class MADDPG_policy:
    ...
    def act(self,obs, deterministic=False):
        ...
        if deterministic == True:
            ...
            actions = self.actor.act(obs).detach()
            actions = torch.clamp(actions + self.act_noise * torch.randn(actions.shape), -self.act_limit, self.act_limit)
...
class MADDPG():
    ...
    def cal_value_loss(self, data,nid):
        ...
        q = self.policy[nid].critic.q(sobs,sact)
        ...
        with torch.no_grad():
            ...
            # Target Q-values
            q_pi_targ = self.policy[nid].critic.q(sobs2, a2)
    ...
    def cal_pi_loss(self, data,id):
        ...
        q_pi = self.policy.critic.q(sobs, actions)
        pi_loss = -q_pi.mean()
    ...
    def train(self, buffer):
        ...
        for indices in batch[:self.num_learning_epochs]:   # TODO : 训练嵌入了batch

            samples = []

            for id in range(self.num_agents):

                obs_batch = buffer[id].observations.view(-1, *buffer.observations.size()[2:])[indices]
                nextobs_batch = buffer[id].next_observations.view(-1, *buffer.observations.size()[2:])[indices]
                states_batch = buffer[id].states.view(-1, *buffer.states.size()[2:])[indices]
                next_states_batch = buffer[id].next_states.view(-1, *buffer.next_states.size()[2:])[indices]
                actions_batch = buffer[id].actions.view(-1, buffer.actions.size(-1))[indices]
                share_actions_batch = buffer[id].share_actions.view(-1, buffer.share_actions.size(-1))[indices]
                rewards_batch = buffer[id].rewards.view(-1, 1)[indices]
                dones_batch = buffer[id].dones.view(-1, 1)[indices]

                sample = {'obs': obs_batch,
                          'sobs': states_batch,
                          'act':actions_batch,
                          'sact':share_actions_batch,
                          'r':rewards_batch,
                          'obs2':nextobs_batch,
